                ...
                if (_instance == null)
                {
                    lock (SyncRoot)
                    {
                        if (_instance == null)
                        {
                            ...
                            {
                                throw new TypeLoadException(string.Format(CultureInfo.CurrentCulture, "The type '{0}' singleton tasarým örüntüsünde private constructor kullanýlmalý.", theType.FullName), ex);
                            }
                        }
                    }
                }
